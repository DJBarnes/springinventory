...
InventoryApp.on('initialize:after', function() {
  ...
  tpl.loadTemplates(['currentInventoryTab','placeOrderTab','viewLogTab','viewOrdersTab'], function() {
    ...
    InventoryApp.ViewLogTab.logCollection.fetch({success : function() {
      var result = Backbone.history.start({pushState: true, root: "/barnesbrothers/"});//, silent:true});
    }});
  });
  
  //Register some handlebars helpers.
  Handlebars.registerHelper("formatDate", function(createdAt) {
	  return new Date(createdAt);
  });
  
  Handlebars.registerHelper("showOpen", function(status) {
	  if(status) {
		  return "Yes";
	  } else {
		  return "No";
