    ...
    _createTempOrders : function() {
      ...
      $('.orders tr').each(function() {
        ...
        if (quantity > 0) {
          ...
          var onOrderQuantity = model.get('on_order_quantity');
        ...
        App.ViewOrdersTab.currentOrders.create(order, {
          ...
          success : function() {
            ...
            _.each(order.get('items'), function(item) {
              ...
              if (!currentItem.saveItem({'on_order_quantity':newOnOrderQty, 'adjustmentQty':Number(currentItem.get('adjustmentQty'))})) {
              ...
              App.ViewLogTab.ViewLogController.logTransaction('Quantity of ' + item.order_qty + ' On Order From Placed Order', App.InventoryTab.currentInventory.where({'id' : Number(item.item_id)})[0]);
            });
